...
for (let i=0; i < inventory.length; i++) {
  let lastCar = 0;
  if (inventory.indexOf(inventory[i]) === inventory.length - 1) {
    lastCar = inventory[i];
    console.log('Car is a ' + lastCar.car_make + " " + lastCar.car_model);
  }
...
//new array from the dealer data containing only the car years and log the result
//in the console.
let carYears = [];
for (let i=0; i < inventory.length; i++) {
...
//the BMWAndAudi array, use JSON.stringify() to show the results of the array
//in the console.
let BMWAndAudi = [];
for (let i=0; i < inventory.length; i++) {
